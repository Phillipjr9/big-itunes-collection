    ...
    document.getElementById("saveSettingsBtn").onclick = () => {
      ...
      const settings = {
        ...
        freeShip: document.getElementById("settingFreeShip").value,
        lowStock: document.getElementById("settingLowStock").value,
        reorderPoint: document.getElementById("settingReorderPoint").value,
        reorderQty: document.getElementById("settingReorderQty").value
      };
      localStorage.setItem("bitc_admin_settings", JSON.stringify(settings));
      setLowStockThreshold(settings.lowStock);
      setDefaultReorderPoint(settings.reorderPoint);
      setDefaultReorderQty(settings.reorderQty);
      logAudit("settings.update", "settings", `Updated settings (reorder point ${settings.reorderPoint}, qty ${settings.reorderQty})`, settings);
      showToast("Settings saved");
      scanAllReorderPoints();
      if (document.getElementById("view-inventory")?.classList.contains("active")) renderInventory();
    };
    document.getElementById("clearOrdersBtn").onclick = () => {
      if (!requirePermission("orders.clear")) return;
      if (confirm("Clear all orders? This cannot be undone.")) {
        const count = getOrders().length;
        saveOrders([]);
        logAudit("order.clear_all", "order", `Cleared all orders (${count} removed)`, { count });
