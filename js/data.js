...
const PRODUCTS = [
  {
    ...
    description: "A fluid silk midi dress in soft rose tones.",
  ...
  },
  {
    ...
    description: "Lightweight and feminine soft blouse.",
  ...
  },
  {
    ...
    description: "High-waisted wide-leg tailored trousers.",
  ...
  },
  {
    ...
    description: "Coordinated two-piece set.",
  ...
  },
  {
    ...
    description: "Flattering wide-leg jumpsuit.",
  ...
  },
  {
    ...
    description: "Softly structured pleated midi skirt.",
  ...
  },
  {
    ...
    description: "Statement sequin mini dress.",
  ...
  },
  {
    ...
    description: "Coordinated crop and skirt set.",
  ...
  },
  {
    ...
    description: "Clean structured office sheath dress.",
  ...
  },
  {
    ...
    description: "Breathable linen everyday shirt.",
    fabric: "100% linen",
  ...
  },
  {
    ...
    description: "High-rise flare stretch denim.",
  ...
  },
  {
    ...
    description: "Floaty vacation maxi dress.",
...
(function mergeCustomProducts() {
  try {
    ...
    custom.forEach(p => {
      ...
      else if (p && p.id != null) map.set(p.id, p);
    });
    PRODUCTS.length = 0;
...
CATEGORIES.forEach(c => { c.count = PRODUCTS.filter(p => p.category === c.id).length; });
...
function addToCart(productId, size, colorHex, qty = 1) {
  ...
  if (existing) existing.qty += qty;
    ...
    cart.push({
      id: productId, name: product.name, price: product.price, image: product.images[0],
      size, colorHex: colorObj.hex, colorName: colorObj.name, qty, slug: product.slug
    });
  }
  saveCart(cart);
  ...
  try {
    ...
    (items || []).forEach(item => {
      ...
      movements.unshift({
        ...
        at: new Date().toISOString(), productId: p.id, productName: p.name, size: item.size,
        from, to, delta: to - from, type: "sale", reason: "Order placed",
        by: "system", byName: "Storefront", role: null, meta: { orderId }
      });
    });
    const toSave = [];
    map.forEach(p => {
...
function toggleWishlist(productId) {
  ...
  else { list.push(productId); showToast("Added to wishlist ❤️"); }
  saveWishlist(list);
  return list;
}
...
function updateNavBadges() {
  ...
  document.querySelectorAll("[data-cart-count]").forEach(el => {
    if (cartCount > 0) { el.textContent = cartCount; el.classList.remove("hidden"); }
    else el.classList.add("hidden");
  });
  document.querySelectorAll("[data-wish-count]").forEach(el => {
    if (wish.length > 0) { el.textContent = wish.length; el.classList.remove("hidden"); }
    else el.classList.add("hidden");
  });
}
  ...
  return `
    <article class="product-card" data-id="${p.id}">
      ...
      <div class="product-card-body">
        ...
        <div class="product-colors">${p.colors.map(c => `<span class="color-dot" style="background:${c.hex}" title="${c.name}"></span>`).join("")}</div>
